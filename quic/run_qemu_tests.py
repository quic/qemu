...
TOOLS_REL_BASE = (
    "/prj/dsp/qdsp6/release/"
    if os.path.exists("/prj/dsp/qdsp6/release/internal/HEXAGON/")
    else "/prj/qct/llvm/release/"
)
HEX_TOOLS_REL_BASE = os.path.join(TOOLS_REL_BASE, "internal/HEXAGON")
LATEST_HEX_TOOLS_REL_BASE = os.path.join(
    HEX_TOOLS_REL_BASE, "branch-8.7/linux64/latest/Tools"
)
LATEST_HEX_TOOLS_BIN = os.path.join(LATEST_HEX_TOOLS_REL_BASE, "bin")
HEX_LLDB = os.path.join(LATEST_HEX_TOOLS_BIN, "hexagon-lldb")

DEFAULT_TEST_DIR = (
    "/prj/qct/llvm/scratch/llvmops_users/bcain/qemu/qurt_kona_tests"
)
...
class TimedoutProcess(object):
    ...
    def write(self, f):
        f.write(
            b'command: "'
            + bytes(self.__dict__["cmd"], encoding="ascii")
            + b'"\n'
        )
    ...
    with Popen(cmd_args, **kwargs) as process:
        try:
            ...
            raise TimeoutExpired(
                process.args, timeout_sec, output=out_text, stderr=err_text
            )

        pass_fail = "PASS" if process.returncode == 0 else "FAIL"
        print(f"\t{pass_fail}: {test}")
        return subprocess.CompletedProcess(
            cmd_args, process.returncode, output, err
        )
...
def report_results(results):
    ...
    passes = sorted(
        [
            test_name(test)
            for test, expired, res, _ in results
            if not expired and res.returncode == 0
        ]
    )
    fails = sorted(
        [
            test_name(test)
            for test, expired, res, _ in results
            if expired or (not expired and res.returncode != 0)
        ]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            os.getcwd(), "qurt_test_" + time.strftime("%Y%b%d_%H%M%S")
        ),
        required=False,
    )
    parser.add_argument(
    ...
    test_paths = [
        (t, get_timeout(t))
        for t in glob(os.path.join(args.binary_dir, "*.pbn"))
    ]
    prog = os.path.abspath(
        os.path.join(args.qemu_bin_dir, "qemu-system-hexagon")
    )
    ...
    def run_tests(t_cfg):
        ...
        gdb_args = (
            "-gdb tcp::{}".format(port_num) if args.debug_timeouts else ""
        )

        cmd = "{} {} -display none -monitor none -m 4G -no-reboot -kernel {}".format(
            prog, gdb_args, t
        )
        ...
        try:
            ...
            return (
                ...
                True,
                TimedoutProcess(
                    cmd=cmd, stdout=e.stdout, stderr=e.stderr, dur_sec=dur_sec
                ),
                dur_sec,
            )
    ...
    skipped = [
        t
        for t in test_paths
        if any(skip_term in t[0] for skip_term in SKIP_LIST_TERMS)
    ]
    solo = [
        t
        for t in test_paths
        if any(solo_term in t[0] for solo_term in SOLO_LIST_TERMS)
    ]
