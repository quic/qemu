...
static char vtcm_buffer[VTCM_SIZE_KB*VTCM_BYTES_PER_KB] __attribute__((aligned(0x10000)));
#endif
...
void create_offsets_and_values_16_32(void)
{
	...
	for(i = 0; i < MATRIX_SIZE; i++)
	{
		...
		letter++;
		// reset to 'A'
		if(letter == 'P') letter = 'D';
	}

  //dump_buf("word_offsets", word_offsets, sizeof(*word_offsets), sizeof(word_offsets));
  //dump_buf("half_offsets", half_offsets, sizeof(*half_offsets), sizeof(half_offsets));
}
...
void vector_scatter_q_16(void)
{
	...
	// do the scatter
	Q6_vscatter_QRMVhV(preds, VTCM_SCATTER16_ADDRESS, region_len, offsets, values);
...
void vector_scatter_acc_32(void)
{
	...
	// do the scatter
	Q6_vscatteracc_RMVwV(VTCM_SCATTER32_ADDRESS, region_len, offsetslo, valueslo);
	Q6_vscatteracc_RMVwV(VTCM_SCATTER32_ADDRESS, region_len, offsetshi, valueshi);
...
void vector_scatter_q_32(void)
{
	...
	// do the scatter
	Q6_vscatter_QRMVwV(predslo, VTCM_SCATTER32_ADDRESS, region_len, offsetslo, valueslo);
	Q6_vscatter_QRMVwV(predshi, VTCM_SCATTER32_ADDRESS, region_len, offsetshi, valueshi);
...
void vector_scatter_16_32(void)
{
  ...
  //print_vector("scatter16_32_address", (HVX_Vector *)VTCM_SCATTER16_32_ADDRESS);
...
void vector_scatter_acc_16_32(void)
{
  ...
  //print_vector("values", (HVX_Vector *)&values);
	...
	// do the scatter
	Q6_vscatteracc_RMWwV(VTCM_SCATTER16_32_ADDRESS, region_len, offsets, values);
  //print_vector("scatter16_32_address", (HVX_Vector *)VTCM_SCATTER16_32_ADDRESS);
...
void vector_scatter_q_16_32(void)
{
  ...
  //print_vector("values", (HVX_Vector *)&values);
	...
	// do the scatter
	Q6_vscatter_QRMWwV(preds, VTCM_SCATTER16_32_ADDRESS, region_len, offsets, values);
  //print_vector("scatter16_32_address", (HVX_Vector *)VTCM_SCATTER16_32_ADDRESS);
...
void vector_gather_q_16(void)
{
	...
	*vgather = Q6_Vh_vsplat_R(gather_q_16_init());
	// do the gather to the gather16 buffer
	Q6_vgather_AQRMVh( vgather, preds, VTCM_SCATTER16_ADDRESS, region_len, offsets );
...
void vector_gather_q_32(void)
{
	...
	*vgatherhi = Q6_Vh_vsplat_R(gather_q_32_init());
	// do the gather to vgather
	Q6_vgather_AQRMVw( vgatherlo, predslo, VTCM_SCATTER32_ADDRESS, region_len, offsetslo );
	Q6_vgather_AQRMVw( vgatherhi, predshi, VTCM_SCATTER32_ADDRESS, region_len, offsetshi );
...
void vector_gather_q_16_32(void)
{
	...
	*vgather = Q6_Vh_vsplat_R(gather_q_16_init());
	// do the gather to vgather
	Q6_vgather_AQRMWw(vgather, preds, VTCM_SCATTER16_32_ADDRESS, region_len, offsets);
...
//  printf("check buffer %s 0x%x, 0x%x, %d\n", name, check, ref, size);
  for (i = 0; i < size; i++) {
    if (check[i] != ref[i]) {
      printf("Error %s [%d]: 0x%x (%c) != 0x%x (%c)\n", name, i, check[i], check[i], ref[i], ref[i]);
      err++;
    }
  }
}
...
{
  memset(vscatter16_ref, FILL_CHAR, SCATTER_BUFFER_SIZE * sizeof(unsigned short));
  scalar_scatter_16(vscatter16_ref);
  check_buffer("check_scatter_16", vscatter16, vscatter16_ref, SCATTER_BUFFER_SIZE * sizeof(unsigned short));
}
...
void scalar_scatter_q_16(unsigned short *vscatter16)
{
	...
	for(i=0; i < MATRIX_SIZE; i++)
	{
		if (half_predicates[i]) vscatter16[half_offsets[i]/2] = half_q_values[i];
	}
...
void check_scatter_acc_16()
{
  ...
  check_buffer("check_scatter_acc_16", vscatter16, vscatter16_ref, SCATTER_BUFFER_SIZE * sizeof(unsigned short));
}
...
void check_scatter_q_16()
{
  ...
  check_buffer("check_scatter_q_16", vscatter16, vscatter16_ref, SCATTER_BUFFER_SIZE * sizeof(unsigned short));
}
...
void scalar_scatter_q_32(unsigned int *vscatter32)
{
	...
	for(i=0; i < MATRIX_SIZE; i++)
	{
		if (word_predicates[i]) vscatter32[word_offsets[i]/4] = word_q_values[i];
	}
...
{
  memset(vscatter32_ref, FILL_CHAR, SCATTER_BUFFER_SIZE * sizeof(unsigned int));
  scalar_scatter_32(vscatter32_ref);
  check_buffer("check_scatter_32", vscatter32, vscatter32_ref, SCATTER_BUFFER_SIZE * sizeof(unsigned int));
}
...
void check_scatter_acc_32()
{
  ...
  check_buffer("check_scatter_acc_32", vscatter32, vscatter32_ref, SCATTER_BUFFER_SIZE * sizeof(unsigned int));
}
...
void check_scatter_q_32()
{
  ...
  check_buffer("check_scatter_q_32", vscatter32, vscatter32_ref, SCATTER_BUFFER_SIZE * sizeof(unsigned int));
}
...
void scalar_scatter_q_16_32(unsigned short *vscatter16_32)
{
	...
	for(i=0; i < MATRIX_SIZE; i++)
	{
		if (half_predicates[i]) vscatter16_32[word_offsets[i]/2] = half_q_values[i];
	}
...
{
  memset(vscatter16_32_ref, FILL_CHAR, SCATTER_BUFFER_SIZE * sizeof(unsigned short));
  scalar_scatter_16_32(vscatter16_32_ref);
  check_buffer("check_scatter_16_32", vscatter16_32, vscatter16_32_ref, SCATTER_BUFFER_SIZE * sizeof(unsigned short));
}
...
void check_scatter_acc_16_32()
{
  ...
  check_buffer("check_scatter_acc_16_32", vscatter16_32, vscatter16_32_ref, SCATTER_BUFFER_SIZE * sizeof(unsigned short));
}
...
void check_scatter_q_16_32()
{
  ...
  check_buffer("check_scatter_q_16_32", vscatter16_32, vscatter16_32_ref, SCATTER_BUFFER_SIZE * sizeof(unsigned short));
}
...
void scalar_gather_q_16(unsigned short *vgather16)
{
	...
	for(i=0; i < MATRIX_SIZE; ++i)
	{
		if (half_predicates[i]) vgather16[i] = vscatter16[half_offsets[i]/2];
	}
...
void check_gather_16()
{
  ...
  check_buffer("check_gather_16", vgather16, vgather16_ref, MATRIX_SIZE * sizeof(unsigned short));
}

void check_gather_q_16()
{
  memset(vgather16_ref, gather_q_16_init(), MATRIX_SIZE * sizeof(unsigned short));
  scalar_gather_q_16(vgather16_ref);
  check_buffer("check_gather_q_16", vgather16, vgather16_ref, MATRIX_SIZE * sizeof(unsigned short));
}
...
void scalar_gather_q_32(unsigned int *vgather32)
{
	...
	for(i=0; i < MATRIX_SIZE; ++i)
	{
		if (word_predicates[i]) vgather32[i] = vscatter32[word_offsets[i]/4];
	}
...
void check_gather_32(void)
{
  ...
  check_buffer("check_gather_32", vgather32, vgather32_ref, MATRIX_SIZE * sizeof(unsigned int));
}

void check_gather_q_32(void)
{
  memset(vgather32_ref, gather_q_32_init(), MATRIX_SIZE * sizeof(unsigned int));
  scalar_gather_q_32(vgather32_ref);
  check_buffer("check_gather_q_32", vgather32, vgather32_ref, MATRIX_SIZE * sizeof(unsigned int));
}
...
void scalar_gather_q_16_32(unsigned short *vgather16_32)
{
	...
	for(i=0; i < MATRIX_SIZE; ++i)
	{
		if (half_predicates[i]) vgather16_32[i] = vscatter16_32[word_offsets[i]/2];
	}
...
void check_gather_16_32(void)
{
  ...
  check_buffer("check_gather_16_32", vgather16_32, vgather16_32_ref, MATRIX_SIZE * sizeof(unsigned short));
}

void check_gather_q_16_32(void)
{
  memset(vgather16_32_ref, gather_q_16_init(), MATRIX_SIZE * sizeof(unsigned short));
  scalar_gather_q_16_32(vgather16_32_ref);
  check_buffer("check_gather_q_16_32", vgather16_32, vgather16_32_ref, MATRIX_SIZE * sizeof(unsigned short));
}
...
#endif /* __linux __ */
#else
  void *vtcm = calloc(1024 * 1024 * 8, sizeof(char));  // 8mb largest vtcm for now
  if (!vtcm) {
