...
static void gen_log_sreg_write(int rnum, TCGv val)
{
    if (rnum < HEX_SREG_GLB_START) {
        ...
        TCGv_i32 num = tcg_constant_i32(rnum);
...
static void gen_read_sreg(TCGv dst, int reg_num)
{
    if (reg_num < HEX_SREG_GLB_START) {
        if (reg_num == HEX_SREG_BADVA) {
            TCGv_i32 num = tcg_constant_i32(reg_num);
...
static void gen_read_sreg_pair(TCGv_i64 dst, int reg_num)
{
    if (reg_num < HEX_SREG_GLB_START) {
        if (reg_num + 1 == HEX_SREG_BADVA) {
            TCGv num = tcg_constant_tl(HEX_SREG_BADVA);
        ...
        TCGv num = tcg_constant_tl(reg_num);
...
{
    TCGv tmp = tcg_constant_tl(count);
    gen_loop0r(tmp, riV, insn);
}
...
{
    TCGv_i64 one = tcg_constant_i64(0xff);
...
static void gen_pred_call(TCGv pred, bool sense, int pc_off)
{
...
static void gen_pred_callr(TCGv pred, bool sense, TCGv new_pc)
{
    TCGv lsb = tcg_temp_new();
    TCGLabel *skip = gen_new_label();
    tcg_gen_andi_tl(lsb, pred, 1);
...
static void gen_endloop01(void)
{
    TCGv lpcfg = tcg_temp_local_new();
    ...
    tcg_gen_brcondi_tl(TCG_COND_LEU, hex_gpr[HEX_REG_LC0], 1, label3);
    {
        gen_write_new_pc(hex_gpr[HEX_REG_SA0]);
    ...
    {
        gen_write_new_pc(hex_gpr[HEX_REG_SA1]);
                          ...
                          bool aligned)
{
    ...
    for (int i = 0; i < sizeof(MMVector) / 8; i++) {
        tcg_gen_qemu_ld_i64(tmp, src, ctx->mem_idx, MO_UNALN | MO_TEQ);
