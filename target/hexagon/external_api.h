...
#define L2TCM_CLADE_BASE	(320*1024)
...
struct coproc_callback_info_struct {
    coproc_type_info_t coproc_id;
  ...
  paddr_t pc_pa;
	...
	union {
		struct  {
			reg_type_t interlock_reg_type;
			int interlock_reg;
			size2u_t interlock_opcode;
		} interlock_details;
		struct {
			int busy_res_type;
			int busy_res;
...
typedef void (*coproc_cycle_status_callback_t)(system_t *sys, processor_t *proc, mmvec_callback_info_t *info);
...
#if 0
/* We dont use this type of callback any more */
typedef void (*cycle_callback_t) (system_t * sys, processor_t * proc,
								  int threadno, int stepcode,
								  int stall_id);
#endif
...
//    paddr_t cfgbase;
	...
	paddr_t clade2_base;
	int clade2_enable;
	int clade2_interrupt;
	int clade2_stable_assert_hack;
...
int arch_get_coproc_stall_info(processor_t *proc, int context, int n, char *buf);
