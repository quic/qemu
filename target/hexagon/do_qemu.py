...
supported_privs = [
    ...
    'Y2_tlbp', 'Y2_tlbr', 'Y2_tlbw', 'Y5_ctlbw', 'Y5_tlboc', 'Y5_tlbasidi',
...
for tag in tags:
    ...
    f.write( "#endif\n\n" )
realf = open('./target/hexagon/qemu_wrap_generated.h', 'wt')
realf.write(f.getvalue())
realf.close()
f.close()
...
for tag in tags:
    f.write ( "OPCODE(%s),\n" % (tag) )
realf = open('./target/hexagon/opcodes_def_generated.h', 'wt')
realf.write(f.getvalue())
realf.close()
f.close()
...
realf = open('./target/hexagon/op_regs_generated.h','w')
realf.write(f.getvalue())
realf.close()
f.close()
