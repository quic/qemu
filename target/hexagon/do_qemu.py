...
def genptr_free_opn(f,tag, regtype,regid,i):
    if (is_pair(regid)):
...
def genptr_src_read_opn(f, tag, regtype, regid):
    if (is_pair(regid)):
...
def gen_tcg_func(f, tag, regs, imms):
    ...
    for regtype,regid,toss,numregs in regs:
        if (is_read(regid)):
            genptr_src_read_opn(f, tag, regtype, regid)
    ...
    for regtype,regid,toss,numregs in regs:
        genptr_free_opn(f, tag, regtype,regid,i)
        i += 1
    for immlett,bits,immshift in imms:
...
def gen_helper_arg_opn(f, tag, regtype,regid,i):
    if (is_pair(regid)):
        if (is_hvx_reg(regtype)):
    ...
    if (numscalarresults > 1):
        ...
        for regtype,regid,toss,numregs in regs:
            if (is_read(regid)):
                if (is_hvx_reg(regtype) and is_readwrite(regid)):
                    continue
                gen_helper_arg_opn(f, tag, regtype,regid,i)
                i += 1
        for immlett,bits,immshift in imms:
...
##
def gen_qemu(f, tag):
    regs = tagregs[tag] if tagregs[tag] else []
    imms = tagimms[tag] if tagimms[tag] else []
...
def main(args):
    with open(args[1], 'rt') as t_f:
        instructions = json.loads(t_f.read())

    with open(args[2], 'rt') as t_f:
        attribinfo = json.loads(t_f.read())
    ...
    global behdict
    global attribdict
    tagregs = {tag: inst['registers'] for tag, inst in instructions.items()}
    tagimms = {tag: inst['immediates'] for tag, inst in instructions.items()}
    semdict = {tag: inst['semantics'] for tag, inst in instructions.items()}
    behdict = {tag: inst['behavior'] for tag, inst in instructions.items()}
    attribdict = {tag: inst['attributes'] for tag, inst in instructions.items()}

    tags = instructions.keys()
