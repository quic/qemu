...
//    return (((((dma_descriptor_type0_t*)d)->next) & DESC_NEXT_MASK) >> DESC_NEXT_SHIFT);
    return ((dma_descriptor_type0_t*)d)->next;
}
...
//    (((dma_descriptor_type0_t*)d)->next) |= ((v << DESC_NEXT_SHIFT) & DESC_NEXT_MASK);
    ((dma_descriptor_type0_t*)d)->next = v;
}
...
//    return (((((dma_descriptor_type0_t*)d)->src) & DESC_SRC_MASK) >> DESC_SRC_SHIFT);
    return ((dma_descriptor_type0_t*)d)->src;
}
...
//    (((dma_descriptor_type0_t*)d)->dst) |= ((v << DESC_DST_SHIFT) & DESC_DST_MASK);
    ((dma_descriptor_type0_t*)d)->dst = v;
}
...
int HEXAGON_DmaDescriptorLinear_toStr(char *s, const HEXAGON_DmaDescriptorLinear_t *p) {
	int nChars = sprintf(s,
		"0x%" VA_FMT "->0x%" VA_FMT " %uB  src: bypass=%u dlbc=%u  dst: bypass=%u dlbc=%u  order=%u done=%u nextP=0x%" VA_FMT,
...
int HEXAGON_DmaDescriptor2D_toStr(char *s, const HEXAGON_DmaDescriptor2D_t *p) {
	int nChars = sprintf(s,
		"0x%" VA_FMT "->0x%" VA_FMT " %ux%u  src: bypass=%u stride=%u widthOffset=%u  dst: bypass=%u stride=%u widthOffset=%u  transform=0x%u allocation=%u order=%u done=%u nextP=0x%" VA_FMT,
		p->srcAddress, p->dstAddress, p->width, p->height,
		p->srcBypass, p->srcStride, p->srcWidthOffset,
		p->dstBypass, p->dstStride, p->dstWidthOffset,
		p->transform, p->allocation,
		p->order, p->done, p->nextDescPointer
		);
	return(nChars);
}
